...
{
    public class InventoryStockDisburseForUseGateway : DbConnection
    {
        public List<InvStockRequisitionModel> GetInventoryRequisitionList(string userName)
        {
            try
            {
                ...
                query = @"  SELECT a.Id,a.ReqNo,a.ReqDate,a.ReqBy,a.ReqNote,a.ReqTime,b.SubsubPNo,case when a.Status =5 then 'pending' else 'Old' end as Status
	                    FROM tbl_INVSTOCK_REQUISITION as a
	                    LEFT JOIN project as b on a.DeptId=b.IdNo WHERE a.BranchId=" + branchId + " AND a.Status =5  ";
                Con.Open();
