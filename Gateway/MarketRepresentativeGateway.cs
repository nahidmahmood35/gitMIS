...
{
    public class MarketRepresentativeGateway : DbConnection
    {
        public string Save(IdNameForDropdownModel aModel)
        {
            ...
            {
                string msg = "";
                const string query = @"INSERT INTO MarketRepresentative (Name) VALUES (@Name)";
                Con.Open();
        ...
        public string Update(IdNameForDropdownModel aModel)
        {
            ...
            {
                string msg = "";
                const string query = @"UPDATE MarketRepresentative SET Name=@Name WHERE Id=@Id";
                Con.Open();
                var cmd = new SqlCommand(query, Con);
                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@Name", aModel.Name);
        ...
        public List<IdNameForDropdownModel> GetMarketRepresentativeList(int param)
        {
            try
            {
                ...
                if (param != 0)
                {
                    query = @"SELECT Id,Name From MarketRepresentative WHERE Id=@Param";
                }
                else
                {
                    query = @"SELECT Id,Name From MarketRepresentative";
                }
                Con.Open();
                var cmd = new SqlCommand(query, Con);
                cmd.Parameters.Clear();
        ...
        public string Delete(int id)
        {
            ...
            {
                string msg = "";
                const string query = @"DELETE FROM MarketRepresentative WHERE Id=@Id";
                Con.Open();
                var cmd = new SqlCommand(query, Con);
                cmd.Parameters.Clear();
