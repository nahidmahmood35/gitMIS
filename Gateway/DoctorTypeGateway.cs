...
{
    public class DoctorTypeGateway : DbConnection
    {
        ...
        public string Save(IdNameForDropdownModel aModel)
        {
            ...
            {
                string msg = "";
                const string query = @"INSERT INTO DoctorType (Name) VALUES (@Name)";
                Con.Open();
        ...
        public string Update(IdNameForDropdownModel aModel)
        {
            ...
            {
                string msg = "";
                const string query = @"UPDATE DoctorType SET Name=@Name WHERE Id=@Id";
                Con.Open();
                var cmd = new SqlCommand(query, Con);
                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@Name", aModel.Name);
        ...
        public List<IdNameForDropdownModel> GetDoctorTypeList(int param)
        {
            try
            {
                ...
                if (param != 0)
                {
                    query = @"SELECT Id,Name From DoctorType WHERE Id=@Param";
                }
                else
                {
                    query = @"SELECT Id,Name From DoctorType";
                }
                Con.Open();
                var cmd = new SqlCommand(query, Con);
                cmd.Parameters.Clear();
        ...
        public string Delete(int id)
        {
            ...
            {
                string msg = "";
                const string query = @"DELETE FROM DoctorType WHERE Id=@Id";
                Con.Open();
                var cmd = new SqlCommand(query, Con);
                cmd.Parameters.Clear();
