...
{
    public class GenderGateway : DbConnection
    {
        public string Save(IdNameForDropdownModel aModel)
        {
            ...
            {
                string msg = "";
                const string query = @"INSERT INTO Gender (Name) VALUES (@Name)";
                Con.Open();
        ...
        public string Update(IdNameForDropdownModel aModel)
        {
            ...
            {
                string msg = "";
                const string query = @"UPDATE Gender SET Name=@Name WHERE Id=@Id";
                Con.Open();
                var cmd = new SqlCommand(query, Con);
                cmd.Parameters.Clear();
                cmd.Parameters.AddWithValue("@Name", aModel.Name);
        ...
        public List<IdNameForDropdownModel> GetGenderList(int param)
        {
            try
            {
                ...
                if (param != 0)
                {
                    query = @"SELECT Id,Name From Gender WHERE Id=@Param";
                }
                else
                {
